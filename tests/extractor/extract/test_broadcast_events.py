...
class MockKafkaException(KafkaException):
    def __init__(self, error):
        self.args = [MockKafkaExceptionArg(error)]
...
def test_skipping_redis_call_when_batch_cancelled(_):
    ...
    producer.produce_event.side_effect = [None, MockKafkaException(KafkaError.UNKNOWN_TOPIC_OR_PART)]
    analysis = mock.Mock(definition_id="definition_id", resource_id="resource_id")
    redis = mock.MagicMock()
    received_event_data = {"batch_id": "batch_id", "batch_type": "batch"}

    broadcast_events(None, analysis, producer, redis, received_event_data)

    assert producer.produce_event.call_count == 2
    redis.hset.assert_called_once_with("batch:batch_id:counter", "resource:resource_id:loaded", 0)
...
def test_redis_counter_when_batch_empty(_):
    ...
    redis.hset.assert_has_calls(
        [
            mock.call("batch:batch_id:counter", "resource:resource_id:loaded", 0),
            mock.call("batch:batch_id:counter", "resource:resource_id:extracted", 0),
        ]
    )
...
def test_redis_counter_when_no_exception_occurs(_):
    ...
    redis.hset.assert_has_calls(
        [
            mock.call("batch:batch_id:counter", "resource:resource_id:loaded", 0),
            mock.call("batch:batch_id:counter", "resource:resource_id:extracted", 3),
        ]
    )
