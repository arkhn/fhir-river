...
def test_migrate(migrator, state, owner_factory, sql_input_factory):
    ...
    try:
        new_state = migrator.apply_tested_migration(("pyrog", "0013_split_join_left_right"))
    except Exception:
        pytest.fail()
...
@pytest.mark.migration(app_label="pyrog", migration_name="0013_split_join_left_right")
